...
public class Game {
	...
	public void handSetUp(Player player, SystemPrinter printer) {
		printer.arrangeHandInstructions();
		player.printName();
		printer.yourTurn();
	...
	public void play(Player player1, Player player2, SystemPrinter printer) {
		// Main game loop method
		printer.generalInstructions();
		while (player1.cardsRemaining() > 0 || player2.cardsRemaining() > 0) {
			turn(player1, printer);
			turn(player2, printer);
		}
	}

	private void turn(Player player, SystemPrinter printer) {
		player.printName();
		printer.yourTurn();
	...
	private void magicRules(Player player, Card playerCard, Card topCard, int index) {
		if (playerCard.getMagic().cardIsSequential() == false) {
			...
			case startAgain:
				...
				break;
			case burn:
				// Play a burn card (10), empty cardPile, draw a card and select a new card to
				// play.
				playCard(player, index);
				cardPile.clear();
	...
	public void pickUp(Player player) {
		System.out.println("~~~ You cannot beat " + cardPile.get(cardPile.size() - 1).printCard() + ". ~~~");
		System.out.println("~~~ " + player.getName() + " picked up the cards in play. ~~~");
